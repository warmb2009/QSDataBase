'''
读取秦殇目录的资源文件lib
经过三天的努力，成功的破解了目标公司的游戏图片格式xbm,还有他的库文件*.lib,也就是可以轻松的从其中提取图片,哎,竟然没有压缩.太简单了. 时间有限,我就说一下他的xbm格式.
**图片文件的头结构
**
1: 16字节 图片标记:xbm
2: 几个整数 都是: 0
...
**图片文件的图象数据(rle)
** 如果行头的第一个(short)短整形数,是零!!! 那么表示接下来的一个(short)短整形数 就是 ,是 连续的不透明色的个数. 反之,就是透明色的个数. 注意一点: 他们都是短整形数,因为,哎,因为图象的点就是16位的.
'''
import struct
import os
import sys
...
class QinXBM():
    ...
    def initdata(self):
        ...
        self.image_array = np.empty(shape = [self.height, self.width, 4], dtype=int)
        # 定位到数据段
        self.buf.seek(64)
    ...
    def read_image_line_data(self, current_count, zero=True):
        if(zero):
            ...
            for i in range(zero_count):
                self.image_array[self.index, current_count] = [255, 255, 255, 0]
                current_count += 1
